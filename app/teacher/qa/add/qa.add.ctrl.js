...
(function () {
    ...
    function ctrl($scope, $mdDialog, teacherFactory, qaAddDetailFactory, $state, teacherQuizFactory, $log, TeacherCourse) {
        ...
        showAlert('', '请注意，一旦开始时间过，您将不能修改开始时间，但可以修改持续时间');
        ...
        $scope.submit = function (title) {
            ...
            if (!(+date > +curdate))return showAlert(' ', '时间不能小于当前时间');
            if (!(minute > 0))return showAlert('', '无效持续时间');
            console.log('helol');
            $log.log('init submit');
            $log.log(title);
            //delete old
            if (quiz !== null) teacherQuizFactory.removeQuiz(quiz._id, function (error, res) {
                if (error) teacherFactory.showToast("error");
            });
            var list = [];
            var i;
            console.log($scope.questionSet);
