...
angular.module('teacher.quiz', [
  ...
  .config(function ($ocLazyLoadProvider) {
    $ocLazyLoadProvider.config({
        modules: [{
            ...
            files: ['app/teacher/quiz/detail/quizDetail.js']
        }, {
            debug: true,
            name: 'teacher.quiz.post',
            files: ['app/teacher/quiz/post/quizPost.js']
        }]
    })
})
  ...
  .controller('teacherQuizzesCtrl', function ($scope, teacherFactory, TeacherQuiz) {
    var onListCallback = function (err, quizzes) {
        if (err) {return alert('Error getting quizzes list.');}
    ...
    $scope.btnRemoveQuiz = function (quiz) {
        var confirmed = confirm('Remove this quiz?');
        if (confirmed) {
            TeacherQuiz.removeQuiz(quiz._id, function (err, quiz) {
                if (err) {return alert('failed to delete quiz: ' + err.data);}
